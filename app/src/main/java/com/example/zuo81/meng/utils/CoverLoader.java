...
public class CoverLoader {
    ...
    public Bitmap loadThumbnail(LocalMusicBean music) {
        return loadCover(music, Type.THUMBNAIL);
    }

    public Bitmap loadBlur(LocalMusicBean music) {
        return loadCover(music, Type.BLUR);
    }

    public Bitmap loadRound(LocalMusicBean music) {
        return loadCover(music, Type.ROUND);
    }

    private Bitmap loadCover(LocalMusicBean music, Type type) {
        Bitmap bitmap;
    ...
    private String getKey(LocalMusicBean music, Type type) {
        if (music == null) {
            return null;
        }

        if (music.getType() == LocalMusicBean.Type.LOCAL && music.getAlbumId() > 0) {
            return String.valueOf(music.getAlbumId()).concat(type.value);
        } else if (music.getType() == LocalMusicBean.Type.ONLINE && !TextUtils.isEmpty(music.getCoverPath())) {
    ...
    private Bitmap loadCoverByType(LocalMusicBean music, Type type) {
        Bitmap bitmap;
        if (music.getType() == LocalMusicBean.Type.LOCAL) {
